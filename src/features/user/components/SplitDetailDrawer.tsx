...
export const SplitDetailDrawer: React.FC = () => {
  ...
  if (!isDetailDrawerOpen || !selectedSplit) return null;
    ...
    <>
      ...
      <div className="fixed right-0 top-0 h-full w-full max-w-md bg-slate-900 border-l border-slate-800 z-50 flex flex-col shadow-2xl">
        ...
        <div className="p-6 border-b border-slate-800 shrink-0">
          ...
          <h2 className="text-xl font-bold text-white">{selectedSplit.name}</h2>
        </div>
              ...
              {isEditMode && !isValidEdit && (
                <div className="bg-red-500/10 border border-red-500/20 rounded-lg p-3">
                  <p className="text-sm text-red-400">
                    Split must add up to 100% (currently {editTotalPercentage}%)
                  </p>
                </div>
              )}
